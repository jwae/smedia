...
const sortierteEintraege = computed(() =>
  [...eintraege.value].sort((a, b) =>
    String(a[props.titleKey] || "").localeCompare(String(b[props.titleKey] || ""), "de")
  )
);

const neuerTitel = computed(() => props.newLabel || `Neu: ${props.singularLabel}`);
const bearbeitenTitel = computed(() => props.editLabel || `${props.singularLabel} bearbeiten`);

function baueLeeresFormular(eintrag = null) {
  const next = {};
  for (const feld of props.fields) {
    if (eintrag) {
      next[feld.key] = eintrag[feld.key] ?? feld.defaultValue ?? (feld.type === "toggle" ? false : "");
    } else {
      next[feld.key] = feld.defaultValue ?? (feld.type === "toggle" ? false : "");
    }
  }
  return next;
}
...
function setzeFormular(eintrag = null) {
  formular.value = baueLeeresFormular(eintrag);
}
...
function optionenFuerFeld(feld) {
  return resourceMap.value[feld.optionsKey] || [];
}

function normalisierePayload() {
  const payload = {};

  for (const feld of props.fields) {
    const wert = formular.value[feld.key];
    if (feld.type === "select") {
      payload[feld.key] = wert === "" ? null : wert;
    } else {
      payload[feld.key] = wert;
    }
  }
...
async function ladeDaten(bevorzugteId = null) {
  ...
  try {
    const requests = [props.apiRequest(props.endpoint)];
    for (const resource of props.resources) {
      requests.push(props.apiRequest(resource.path));
    }

    const responses = await Promise.all(requests);
    eintraege.value = Array.isArray(responses[0]) ? responses[0] : [];

    const nextResources = {};
    props.resources.forEach((resource, index) => {
      nextResources[resource.key] = Array.isArray(responses[index + 1]) ? responses[index + 1] : [];
    });
    resourceMap.value = nextResources;
...
async function speichern() {
  ...
  try {
    const payload = normalisierePayload();
    const response = istNeu.value
      ? await props.apiRequest(props.endpoint, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        })
      : await props.apiRequest(`${props.endpoint}/${ausgewaehlteId.value}`, {
...
<template>
  <div class="katalog-admin-stack">
    ...
    <div v-if="laedt" class="empty-state">{{ pluralLabel }} werden geladen...</div>
    ...
    <section v-else class="katalog-admin-layout">
      <aside class="einstellungen-records">
        ...
        <div v-if="sortierteEintraege.length === 0" class="empty-state katalog-empty-state">
          Noch keine {{ pluralLabel }} vorhanden.
        </div>

        <button
          v-for="eintrag in sortierteEintraege"
          :key="`${endpoint}-${eintrag.id}`"
          type="button"
          :class="[
        ...
        >
          <span class="einstellungen-record-head">
            <span class="einstellungen-record-title">{{ eintrag[titleKey] }}</span>
      ...
      <div class="einstellungen-form-panel">
        <div class="einstellungen-stack">
          <article class="einstellungen-card">
            <h3>{{ istNeu ? neuerTitel : bearbeitenTitel }}</h3>

            <div class="form-grid">
              <template v-for="feld in fields" :key="feld.key">
                <label v-if="feld.type === 'text'" :class="['field', feld.wide ? 'field-wide' : '']">
                  <span>{{ feld.label }}</span>
                  <input
                    v-model="formular[feld.key]"
                    type="text"
                ...
                <label v-else-if="feld.type === 'textarea'" :class="['field', feld.wide ? 'field-wide' : '']">
                  <span>{{ feld.label }}</span>
                  <textarea
                    v-model="formular[feld.key]"
                    :rows="feld.rows || 4"
                ...
                <label v-else-if="feld.type === 'select'" :class="['field', feld.wide ? 'field-wide' : '']">
                  <span>{{ feld.label }}</span>
                  <select v-model="formular[feld.key]">
                    <option value="">- bitte waehlen -</option>
                    <option
                      v-for="option in optionenFuerFeld(feld)"
                      :key="`${feld.key}-${option[feld.optionValue || 'id']}`"
                      :value="option[feld.optionValue || 'id']"
                    >
                      {{ option[feld.optionLabel || 'bezeichnung'] }}
                    </option>
                  </select>
                </label>

                <label v-else-if="feld.type === 'toggle'" :class="['field', feld.wide ? 'field-wide' : '']">
                  <span>{{ feld.label }}</span>
                  <div class="toggle-row">
                    <input :id="`${endpoint}-${feld.key}`" v-model="formular[feld.key]" type="checkbox" class="toggle-input" />
                    <label :for="`${endpoint}-${feld.key}`" class="toggle-label"><span class="toggle-knob"></span></label>
                    <span class="toggle-text">{{ formular[feld.key] ? "Ja" : "Nein" }}</span>
                  </div>
                </label>
              </template>
            </div>
