...
const sortierteEintraege = computed(() =>
  [...eintraege.value].sort((a, b) => String(a.kategorie || "").localeCompare(String(b.kategorie || ""), "de"))
);
...
async function ladeDaten(bevorzugteId = null) {
  ...
  try {
    const response = await props.apiRequest("/artikel-kategorien");
...
async function speichern() {
  ...
  try {
    const payload = {
      ...
      bemerkung: formular.value.bemerkung
    };

    const response = istNeu.value
      ? await props.apiRequest("/artikel-kategorien", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        })
      : await props.apiRequest(`/artikel-kategorien/${ausgewaehlteId.value}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload)
        });
...
<template>
  <div class="katalog-admin-stack">
    ...
    <div v-if="laedt" class="empty-state">Artikel-Kategorien werden geladen...</div>
      ...
      <aside class="einstellungen-records">
        <div class="einstellungen-records-head">
          ...
          <button
            ...
            title="Neue Artikel-Kategorie"
            aria-label="Neue Artikel-Kategorie"
            @click="neuerEintrag"
          >
        ...
        <div v-if="sortierteEintraege.length === 0" class="empty-state katalog-empty-state">
          Noch keine Artikel-Kategorien vorhanden.
        </div>

        <button
          v-for="eintrag in sortierteEintraege"
          :key="`artikel-kategorie-${eintrag.id}`"
          type="button"
          :class="[
        ...
        >
          <span class="einstellungen-record-head">
            <span class="einstellungen-record-title">{{ eintrag.kategorie }}</span>
      ...
      <div class="einstellungen-form-panel">
        <div class="einstellungen-stack">
          <article class="einstellungen-card">
            <h3>{{ istNeu ? "Neue Artikel-Kategorie" : "Artikel-Kategorie bearbeiten" }}</h3>

            <div class="form-grid">
              <label class="field">
                <span>Kategorie</span>
                <input v-model="formular.kategorie" type="text" maxlength="150" placeholder="z. B. Tablet" />
              </label>
              ...
              <label class="field">
                ...
                <div class="toggle-row">
                  <input
                    id="artikel-kategorie-aktiv"
                    v-model="formular.aktiv"
                    type="checkbox"
                    class="toggle-input"
                  />
                  <label for="artikel-kategorie-aktiv" class="toggle-label"><span class="toggle-knob"></span></label>
